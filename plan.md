...
### 6e 착수 직후 의도적 CORS 검증 (중요)

미니 프로젝트는 `next.config.js`의 `rewrites`로 `/api/*` → `http://localhost:8000/api/*`
프록시를 걸었기 때문에 브라우저 입장에서는 동일 출처(`localhost:3000`)다.
즉 **백엔드 `cors_origins` 설정이 실검증되지 않은 상태**다.

6e 본편 첫 번째 작업으로 다음을 수행:
1. `web/next.config.js`의 `rewrites`를 제거
2. `.env.local`에 `NEXT_PUBLIC_API_URL=http://localhost:8000` 추가
3. fetch 호출을 `${process.env.NEXT_PUBLIC_API_URL}/api/stock/005930/ohlcv`로 전환
4. 브라우저에서 동작 확인 — 이때 CORS preflight가 처음으로 실제 발사됨
5. 실패 시 `src/portfolio_report/config.py`의 `cors_origins` 기본값에
   `http://localhost:3000`이 포함되어 있는지 확인하고 조정

실패가 6e 중반에 우연히 발견되면 디버깅 범위가 퍼지므로,
**본편 첫 커밋 전에 의도적으로 한 번 터뜨려두는** 게 목표.
